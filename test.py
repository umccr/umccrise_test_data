...
gunzip {ref_fasta_path}.gz''')
    ...
    def test(self, docker_wrapper_mode=False, skip_pcgr=False):
        ...
        for T, B in zip(TUMORS, BATCHES):
            ...
            failed = self._check_file(failed, f'{results_dir}/{key}/pcgr/input/{key}-somatic-cna.tsv'                                                   , wrapper='cut -f1,2,3')
            if loc.name in ['spartan', 'raijin'] and not skip_pcgr:
                failed = self._check_file(failed, f'{results_dir}/{key}/pcgr/{key}-somatic.pcgr.html'                                 , check_diff=False)
                failed = self._check_file(failed, f'{results_dir}/{key}/pcgr/{key}-normal.cpsr.html'                                  , check_diff=False)
            failed = self._check_file(failed, f'{results_dir}/{key}/{key}-rmd_report.html'                                            , check_diff=False)
            failed = self._check_file(failed, f'{results_dir}/{key}/small_variants/{key}-normal-ensemble-predispose_genes.vcf.gz'     , vcf_ignore_lines, check_diff=False)
            failed = self._check_file(failed, f'{results_dir}/{key}/small_variants/{key}-normal-ensemble-predispose_genes.vcf.gz.tbi' , check_diff=False)
